...
import { createModule2Session } from '../lib/module2SessionService';
...
import { module2InstructionSteps } from '../config/module2InstructionSteps';
import { module2InstructionVisualForStepId } from './module2/Module2InstructionVisuals';
...
const Module2Instructions = () => {
  ...
  const decorateStep = useCallback((step: InstructionStep): InstructionStep => {
    return {
      ...step,
      visual: module2InstructionVisualForStepId(step.id),
    };
  }, []);
  ...
  const handleStartModule = useCallback(async () => {
    setStarting(true);
    try {
      const result = await createModule2Session();
      if (!result.ok) {
        alert(result.error);
        return;
      }
      navigate('/module/2/camera-control', { state: { sessionId: result.sessionId } });
    } finally {
      setStarting(false);
    }
  }, [navigate]);
      ...
      <div className="flex" style={{ minHeight: 'calc(100vh - 88px)' }}>
        ...
        <main className="flex-1 min-w-0" style={{ padding: 'clamp(20px, 4vw, 48px)' }}>
          <div className="max-w-4xl mx-auto">
            ...
            >
              Quick onboarding—five short steps before you begin.
            </p>

            <ModuleInstructionFlow
              steps={module2InstructionSteps}

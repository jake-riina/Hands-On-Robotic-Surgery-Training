...
const IncompleteModule2 = () => {
  const navigate = useNavigate();
  const location = useLocation();

  const state = (location.state as { orbsCollected?: number; totalOrbs?: number } | null) ?? {};
  const orbsCollected = Math.max(0, state.orbsCollected ?? 0);
  const totalOrbs = Math.max(1, state.totalOrbs ?? 5);
  const percentage = Math.round((orbsCollected / totalOrbs) * 100);
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#26313E' }}>
      ...
      >
        <div className="flex h-full min-h-0 flex-1 items-center min-w-0">
      ...
      <div className="flex" style={{ minHeight: 'calc(100vh - 88px)' }}>
        ...
        <main className={`flex-1 ${styles.mainContentWrapper}`}>
          <div className="max-w-2xl mx-auto px-4" style={{ paddingLeft: '3rem', paddingRight: '3rem', paddingTop: '0.5rem', paddingBottom: '3rem', transform: 'translateX(-48px)' }}>
            <div className="text-center mb-16">
              <h1 className="text-4xl font-bold mb-6" style={{ color: 'white' }}>
                Module Incomplete
              </h1>
              <p className="text-xl leading-relaxed" style={{ color: '#E5E7EB' }}>
                You did not collect all the orbs in the allotted time.
              </p>
            </div>
            ...
            <div className="flex flex-col items-center gap-16">
              <div className="flex flex-col items-center" style={{ marginTop: '2rem' }}>
                ...
                <p className="mt-6 text-lg" style={{ color: '#9CA3AF' }}>
                  Orbs collected: {orbsCollected} / {totalOrbs}
                </p>
              </div>
...
export default IncompleteModule2;
